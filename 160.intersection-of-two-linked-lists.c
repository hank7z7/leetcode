...
{
	// The length of two linked-list
	int length_A = 0, length_B = 0;
	struct ListNode *curr_head = headA;
	...
	// Check value and two node must be same
	while (headA->val != headB->val || headA != headB) {
		headA = headA->next;
		headB = headB->next;
		// For no intersection
		if (headA == NULL)
...
// Your runtime beats 97.16 % of c submissions
// Your memory usage beats 72.98 % of c submissions (13.9 MB)